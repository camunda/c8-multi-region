...
module "eks_cluster_region_0" {
  ...
  availability_zones        = local.owner.availability_zones
...
module "eks_cluster_region_1" {
  ...
  np_desired_node_count     = var.np_desired_node_count
  single_nat_gateway        = var.single_nat_gateway
  availability_zones        = local.accepter.availability_zones
  availability_zones_count  = 2
...
module "eks_cluster_region_2" {
  ...
  region                    = local.quorum.region
  name                      = "${var.cluster_name}-${local.quorum.region_full_name}"
  kubernetes_version        = var.kubernetes_version
  np_instance_types         = var.np_instance_types
  np_capacity_type          = var.np_capacity_type
  np_max_node_count         = var.np_max_node_count
  np_desired_node_count     = var.np_desired_node_count
  single_nat_gateway        = var.single_nat_gateway
  availability_zones        = local.quorum.availability_zones
  availability_zones_count  = 2
  cluster_service_ipv4_cidr = local.quorum.service_cidr_block
  cluster_node_ipv4_cidr    = local.quorum.vpc_cidr_block
