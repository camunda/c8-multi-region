...
echo "Generating CoreDNS config entries for namespaces:
  Cluster 0 namespace: $namespace_0
  Cluster 1 namespace: $namespace_1
  Cluster 2 namespace: $namespace_2
"
...
if [ -z "$namespace_2" ]; then
    read -r -p "Enter the Kubernetes cluster namespace where Camunda 8 is installed, in region 2: " namespace_2
fi
...
if [ -z "$IPS_0" ] || [ -z "$IPS_1" || [ -z "$IPS_2" ]; then
...
config_for_cluster_0=$(generate_string "$namespace_1" "$internal_lb_1" "$namespace_2" "$internal_lb_2")
config_for_cluster_1=$(generate_string "$namespace_0" "$internal_lb_0" "$namespace_2" "$internal_lb_2")
config_for_cluster_2=$(generate_string "$namespace_0" "$internal_lb_0" "$namespace_1" "$internal_lb_1")
...
echo "CoreDNS config entries generated successfully. Check dns config command"

echo "kubectl --context $CLUSTER_0 logs -f deployment/coredns -n kube-system"
echo "kubectl --context $CLUSTER_1 logs -f deployment/coredns -n kube-system"
echo "kubectl --context $CLUSTER_2 logs -f deployment/coredns -n kube-system"
