...
if ((cluster_size < 3)); then
    echo "Cluster size $cluster_size is too small and should be at least 3. A multi-region setup is not recommended for a small cluster size."
    exit 1
fi
...
initial_contact=$(generate_initial_contact "$namespace_0" "$namespace_1" "$namespace_2" "$helm_release_name" "$cluster_size")
...
elastic2=$(generate_exporter_elasticsearch_url "$namespace_2" "$helm_release_name")
...
echo "  value: $elastic1"
echo
echo "Please use the following to change the existing environment variable ZEEBE_BROKER_EXPORTERS_CAMUNDAREGION2_ARGS_CONNECT_URL $target_text. It's part of the 'zeebe.env' path."
echo
echo "- name: ZEEBE_BROKER_EXPORTERS_CAMUNDAREGION2_ARGS_CONNECT_URL"
echo "  value: $elastic2"