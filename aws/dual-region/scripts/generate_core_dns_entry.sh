...
IPS_0=$(aws ec2 describe-network-interfaces --region "$REGION_0" --filters Name=description,Values="ELB net/${HOST_0}/${HOST_1}" --query  'NetworkInterfaces[*].PrivateIpAddress' --output text --no-cli-pager)
IPS_1=$(aws ec2 describe-network-interfaces --region "$REGION_1" --filters Name=description,Values="ELB net/${HOST_1}/${HOST_0}" --query  'NetworkInterfaces[*].PrivateIpAddress' --output text --no-cli-pager)
